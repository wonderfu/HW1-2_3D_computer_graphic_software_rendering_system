...
void TForm1::Lighting(struct Vertex vertex, Node L_ray, Node V_ray, double dis, double* color)
{
	...
	color[0] = vertex.ka[0] * light0.sa[0] * light0.sd[0] + Att * light0.color[0] * (vertex.kd[0] * light0.sd[0] * (N * L) + vertex.ks[0] * light0.ss[0] * pow((N * H), n));
	color[1] = vertex.ka[1] * light0.sa[1] * light0.sd[1] + Att * light0.color[1] * (vertex.kd[1] * light0.sd[1] * (N * L) + vertex.ks[1] * light0.ss[1] * pow((N * H), n));
