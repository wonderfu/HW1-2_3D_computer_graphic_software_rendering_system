...
void TForm1::Lighting(struct Vertex vertex, Node L_ray, Node V_ray, double dis)
{
        int n = 2;
	...
	vertex.color[0] = vertex.ka[0] * ambient_light + Att * light0.color[0] * (vertex.kd[0] * (N * L) + vertex.ks[0] * pow((N * H), n));
	vertex.color[1] = vertex.ka[1] * ambient_light + Att * light0.color[1] * (vertex.kd[1] * (N * L) + vertex.ks[1] * pow((N * H), n));
	vertex.color[2] = vertex.ka[2] * ambient_light + Att * light0.color[2] * (vertex.kd[2] * (N * L) + vertex.ks[2] * pow((N * H), n));
}

TForm1::Node TForm1::Interpolation(Node V1, Node V2, Node V3, double t, double u, double v)
{ 
	Node renode;
